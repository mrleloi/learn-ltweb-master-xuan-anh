...
Media.prototype.getInnerHTML = function (objectId) {
    ...
    var advsOverClipId = 'id="advsoverclip"';
    //alert(advsOverClipId);
    if (FCK.innerHTML.indexOf(advsOverClipId) >= 0) {
        var dem = 1;
        advsOverClipId = 'id="advsoverclip' + dem + '"';
        while (FCK.innerHTML.indexOf(advsOverClipId) >= 0) {
            dem++;
            advsOverClipId = 'id="advsoverclip' + dem + '"';
        }
    }
    ...
    s += '<table cellspacing="0" cellpadding="0" border="0" align="' + this.align + '"><tr><td ' + advsOverClipId + '><p id="player' + randomnumber + '">Tải chương trình xem <a href="http://www.macromedia.com/go/getflashplayer">tại đây</a> để xem nội dung tệp này.</p></td></tr></table>';
